...
def test_ems_infodva_runner_forwards_exact_3x3_and_ante_infodva() -> None:
    args = ems_infodva.build_parser().parse_args([])

    forwarded = ems_infodva.build_forward_args(args, [])

    assert forwarded[forwarded.index("--solver") + 1] == "exact"
    assert forwarded[forwarded.index("--coverage-radius-km") + 1 : forwarded.index("--facility-budget")] == [
        "1.0",
        "2.0",
        "3.0",
    ]
    assert forwarded[forwarded.index("--facility-budget") + 1 : forwarded.index("--holdout-hours")] == [
        "3",
        "5",
        "8",
    ]
